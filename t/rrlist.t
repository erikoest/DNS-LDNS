...
# Create list
my $list = new Net::LDNS::RRList;
isa_ok($list, 'Net::LDNS::RRList', 'Create an empty rr list');

# Push/pop/count rr
$list->push(new Net::LDNS::RR(str => 'ns.myzone.org 3600 IN AAAA ::1'));
...
$list->push(new Net::LDNS::RR(str => 'ns.myzone.org 7200 IN A 192.168.100.2'));
...
# Push/pop list
my $l2 = new Net::LDNS::RRList;
$l2->push(new Net::LDNS::RR(str => 'ns2.myzone.org 3600 IN A 192.168.100.0'));
$l2->push(new Net::LDNS::RR(str => 'ns2.myzone.org 3600 IN A 192.168.100.1'));
$list->push_list($l2);
...
my $rr = new Net::LDNS::RR(str => 'ns2.myzone.org 3600 IN A 192.168.100.0');
ok($rrset->contains_rr($rr), 'RRSet contains rr '.$rr->to_string);
is($list->compare($l2), -1, '$list < $l2');
is($l2->compare($list), 1, '$l2 > $list');

$list->push(new Net::LDNS::RR(str => 'ns3.myzone.org 3600 IN A 192.168.100.0'),
	    new Net::LDNS::RR(str => 'ns3.myzone.org 3600 IN A 192.168.100.1'),
	    new Net::LDNS::RR(str => 'ns4.myzone.org 3600 IN A 192.168.100.1'));
my $subtype = $list->subtype_by_rdata(
    new Net::LDNS::RData(LDNS_RDF_TYPE_A, '192.168.100.1'), 0);
...
my $keylist = new Net::LDNS::RRList;
$keylist->push(
    new Net::LDNS::RR(str => 'trondheim.no.           3600    IN      DNSKEY  256 3 8 AwEAAZIDdRI8I+F/J6OT8xX7CbGQYRr8rWH9dvloUlRJXcEVE2pRAez6 pJC5Odg+i2WvDUeE4tUO1gwwjU83TIinZxxsDnqr7FzvqpHeJbVd2N3d S4zaJcbjSnwMqdebmTEXSrflp8DeIAH0GQGNQjhOPubbb/nADYP2RS1i CoOADa8P'),
    new Net::LDNS::RR(str => 'trondheim.no.           3600    IN      DNSKEY  257 3 8 AwEAAax9EgKyRsMpU2B0E2dZ+nkWnmZHjlBO3uXBI+2x33dG8bk+XSqr kyWTelhhsqLqIxsaYSwYgzLtn+/qzlFjKwcaU95p+Tp95MOVXYqUtRyC VyLGkzA7ZDbx7TFCi3PyLDM/Arx+DvOx6nNvA/erqIU5gYEo9Nm1KXEy rhfSn3xc96p1AOhmTuSo6EfYlPY4gxHDgJdHFv7Fi9zV6VFmJ29h0rsG 5g3pV1lvCcGcxfRLJ1u7JRw2BWMo9lgHzGuypEVV7iLnvbfDlXhF+jAS owR2JxlESC3dOgNiNWvc4pbyVXBXpP6h/5JpcxkzF7BNJMZiLN14qvam G1+LuZM8qfc=')
);

my $soalist = new Net::LDNS::RRList;
$soalist->push(
    new Net::LDNS::RR(str => 'trondheim.no.           3600    IN      SOA     charm.norid.no. hostmaster.norid.no. 2013021137 14400 1800 2419200 3600')
);

my $siglist = new Net::LDNS::RRList;
$siglist->push(
    new Net::LDNS::RR(str => 'trondheim.no.           3600    IN      RRSIG   SOA 8 2 3600 20130227105101 20130213090318 36381 trondheim.no. NbeN8E4pvQSDk3Dn0i8B4e2A3KAY8JrX+zcJazPTgHbT6wjzCncn3ANn 6rs+HdcCLtptyX1QbzlZD/lOY8kjJw5TEUoFX2Q/2sBYdt1aT6qgt/+H o71iUz3bk1V73zjSG/OpqG0oXmjCWSBZgzK6UI+zGlgG0Kvrc7H1pw5S ZBA=')
);
